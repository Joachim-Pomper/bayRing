...
class WaveformModel(cpnest.model.Model):
    ...
    def __init__(self, t_NR, tM_start, tM_peak, wf_model, N_ds_modes, Kerr_modes, metadata, fit_metadata, qnm_cached, l_NR, m_NR, tail=0, tail_modes=None, redshift_modes=None, quadratic_modes=None, const_params=None, KerrBinary_version = 'London2018', KerrBinary_amp_nc_version = 'bmrg-Jmrg', TEOB_NR_fit = 0, TEOB_template = 'qc', TEOB_qc_fit_type = None):
    ...
    def _KerrBH_model(self, amps, tail_parameters=None, quadratic_modes=None, redshift_modes=None):
    ...
    def TEOBPM_waveform(self, params, fixed_params):
        ...
        # ------------------------------------------
        if self.quad_mode_flag == 1:
            t_q_sigmoid   = get_param(params, f't_q_sigmoid_{self.l_NR}{self.m_NR}', 0.0)
            width_sigmoid = get_param(params, f'width_sigmoid_{self.l_NR}{self.m_NR}', 1.0)
            amp_sigmoid   = get_param(params, f'amp_sigmoid_{self.l_NR}{self.m_NR}', 0.0)
            psi_sigmoid   = get_param(params, f'psi_sigmoid_{self.l_NR}{self.m_NR}', np.pi)
        else:
            # quad_mode_flag == 0 → disable sigmoid entirely
            t_q_sigmoid   = 0.0
            width_sigmoid = 1.0
            amp_sigmoid   = 0.0
            psi_sigmoid   = np.pi
